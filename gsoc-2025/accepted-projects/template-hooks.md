...
DMD is not a performance-oriented compiler, being more of a proof of concept for the D language and providing a reference implementation, without striving to provide the best performance possible. That is why the real performance benefits of these changes will be seen in other compilers, such as LDC (the LLVM-based D compiler) and GDC (the GCC-based D compiler). Changes to the DMD compiler take some time to be integrated into these third-party compilers, so I was unable to truly benchmark the performance of the new hooks. However, I have tested the hooks that have been translated into templates a while ago and are already available in LDC and GDC, so this could give a good sense of what improvements to expect.
